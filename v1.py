...
def build_cell_positions() -> Dict[Tuple[int, int], Optional[int]]:
    ...
    This function extends `build_position_map` by inserting placeholders for
    f-block positions.  Specifically, it leaves the (period, group) cells
    corresponding to lanthanum (57) and actinium (89) empty (set to None) so
    that the caller can insert placeholder elements (e.g. "57–71").

    Returns:
        A dictionary mapping (row, col) -> atomic number or None.
    """
    pos = build_position_map()
    # Insert placeholders at (6,3) and (7,3) to indicate lanthanide and actinide series
    pos[(6, 3)] = None
    pos[(7, 3)] = None
    return pos
...
def render_html(
    ...
    if "legend" in data:
        ...
        for label, color in label_colors.items():
            legend_order.append((label, color))
    # Build base grid: 9 rows x 18 cols (rows 1–9, with lanthanides and actinides)
    ...
    def render_sample_quarter(sample: Sample, idx: int) -> str:
        ...
        # Determine background colour; fallback to neutral if not provided
        bg = sample.color if sample.color else "#eaeaea"
        # Short label for indexing (1–4)
        label = str(idx + 1)
        # Construct tooltip content for this sample
        tooltip = []
        if sample.state:
            tooltip.append(f"{esc(sample.state)}")
        if sample.value:
            tooltip.append(f"{esc(sample.value)}")
        tip_text = esc(" | ".join(tooltip)) if tooltip else ""
        return (
            f'<div class="quarter" style="background:{esc(bg)}" '
            f'data-tip="{tip_text}"><span class="sidx">{esc(label)}</span></div>'
        )
    # Compose cells
    cells_html: List[str] = []
    # Iterate over rows 1–9 for rendering
    for row in range(1, 10):
        for col in range(1, 19):
            z = None
            if row <= 7:
                z = pos_map.get((row, col))
            # Insert f-block elements into rows 8 and 9 (lanthanides and actinides)
            if row == 8 and 4 <= col <= 18:
                idx = col - 4
                if idx < len(lanths):
                    z = lanths[idx]
            ...
            if z is None:
            ...
            elif z == 57 and row == 6 and col == 3:
                # Should not occur because pos_map already None; included for clarity
                cells_html.append('<div class="cell empty"></div>')
            elif z == 89 and row == 7 and col == 3:
                cells_html.append('<div class="cell empty"></div>')
            else:
                # Real element
                e = elem_by_z.get(z)
                if e is None:
                    ...
                    # Build tooltip for element
                    tooltip_lines = [f"{esc(e.name)}", f"Z={e.z}"]
    ...
    for label, color in legend_order:
        if not label:
            continue
        # Ensure colour string begins with '#'
        color_hex = color if color.startswith('#') else color
        legend_html.append(
            f'<div class="legend-item"><span class="legend-colour" '
